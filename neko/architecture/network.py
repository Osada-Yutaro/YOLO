def graph_def(x, keep_prob=1.):
    ...
    def __second_block(x):
        return lf.max_pool(lf.leaky_relu(lf.conv2d(x, [3, 3, 192, 128], 1, name='w_2')), 2, 2)
    def __third_block(x):
        layer_1 = lf.leaky_relu(lf.conv2d(x, [1, 1, 128, 256], name='w_3_1'))
    ...
    def __fourth_block(x):
        layer_1 = lf.leaky_relu(lf.conv2d(x, [1, 1, 256, 512], name='w_4_1'))
    ...
    def __fifth_block(x):
        layer_1 = lf.leaky_relu(lf.conv2d(x, [1, 1, 512, 1024], name='w_5_1'))
    ...
    def __sixth_block(x):
        layer_1 = lf.leaky_relu(lf.conv2d(x, [3, 3, 1024, 1024], name='w_6_1'))
    ...
    def __seventh_block(x, keep_prob):
        ...
        conn = lf.leaky_relu(tf.matmul(tf.reshape(x, [-1, 7*7*1024]), w))
        return tf.nn.dropout(conn, rate=1-keep_prob)
    def __eighth_block(x):
        w = lf.weight_variable([4096, 7*7*30], name='w_8')
        return tf.nn.relu(tf.reshape(tf.matmul(x, w), [-1, 7, 7, 30]), 'result')
