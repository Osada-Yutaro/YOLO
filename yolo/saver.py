def save_model(checkpoint_dir, saved_model_dir):
    ...
    with tf.Session() as sess:
        ...
        def_map = {'serving_default': _build_signature({'input': tf.saved_model.utils.build_tensor_info(x)}, {'result': tf.saved_model.utils.build_tensor_info(y)})}
...
def convert_tflite(saved_model_dir, tflite_dir):
    import os
    import tensorflow as tf
    converter = tf.lite.TFLiteConverter.from_saved_model(saved_model_dir, signature_key='serving_default')
